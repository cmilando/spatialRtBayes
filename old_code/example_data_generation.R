...
g1 <- ggplot(data = R_matrix_dataframe_long, 
             aes(x = t, y = R_t, color = region)) +
  geom_line() +
  xlab("Day") +
...
M_simulated <- matrix(NA, t_max + 1, m)

initial_inf_num = 10
N_simulated[1, ] <- initial_inf_num
M_simulated[1, ] <- initial_inf_num
WREV  <- rev(w)
# .....
for (t in 1:t_max) {
  
  W1 = rev(w[1:min(length(w), t)])
  N1 = N_simulated[c(max((t - (length(w) - 1)), 1):t), , drop = F]
  R1 = diag(R_matrix[t, ])
 ...
 #  # first, get the M*W dot product
  sum_m_w <- vector("numeric", 3)
  start_index = max(1, t - 14 + 1)
  end_index = t # <<<< from t-1 to t
  start_index
  end_index
 #  
 #  ## sum(m*w)
  for (jx in 1:3) {
    sum_m_w[jx] = M_simulated[end_index:start_index, jx] %*% w[1:(end_index - start_index + 1)]

  }
  sum_m_w
  
  cc_calc <- sum_m_w %*% diag(R_matrix[t, ]) %*% P
  cc_calc 

  #N_new == cc_calc

  N_simulated[t + 1, ] <- N_new
  M_simulated[t + 1, ] <- cc_calc
...
g2 <- ggplot(data = plot_dat_long, aes(x = t, y = incidence, color = region)) +
  geom_line() +
  xlab("Day") +
  ylab("Incidence") +
  theme_bw()

M_simulated = as.data.frame(M_simulated)
colnames(M_simulated)<- c("a", "b", "c")
M_simulated$t <- 0:t_max
plot_dat_long <- gather(M_simulated, region, incidence, a:c, factor_key = TRUE)
head(plot_dat_long)

g2b <- ggplot(data = plot_dat_long, aes(x = t, y = incidence, color = region)) +
  geom_line() +
  xlab("Day") +
  ylab("Incidence") +
  theme_bw()
...
ggplot(data = plot_dat_long, aes(x = t, y = incidence, color = region)) +
  geom_line() +
  theme_bw()
...
for (index in 1:10) {
  ...
  for (t in 1:t_max) {
    ...
    ### >>>>>
    cases <- sapply(cases_expectation, function(x) {
      rpois(1, x)
    })
    ### >>>>>
...
head(N_simulated_list[[1]])
head(N_simulated_list[[2]])
